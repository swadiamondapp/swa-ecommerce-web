import React from "react";
import Image from "next/image";

const SwaPromise = () => {
  const data = [
    {
      title: "200+ store",
      description: "200+ store",
      image: "/try/storelogo.svg",
    },
    {
      title: "Life time exchange & buy back",
      description: "Life time exchange & buy back",
      image: "/try/exchange.svg",
    },

    {
      title: "Free shipping",
      description: "Free shipping",
      image: "/try/delivery.svg",
    },

    {
      title: "30 Day money back",
      description: "30 Day money back",
      image: "/try/moneyBack.svg",
    },

    {
      title: "Personalized video consultation",
      description: "Personalized video consultation",
      image: "/try/vedioconsultaion.svg",
    },

    {
      title: "Try at home service",
      description: "Try at home service",
      image: "/try/home.svg",
    },

    {
      title: "BIS Hallmark, IGI, GIA",
      description: "BIS Hallmark, IGI, GIA",
      image: "/try/Bis.svg",
    },

    {
      title: "100% Refund Return with 30 days of delivery",
      description: "100% Refund Return with 30 days of delivery",
      image: "/try/refund.svg",
    },
  ];

  return (
    <div className="w-full bg-[#F8F4E9]">
      <div className="max-w-container h-full mx-auto justify-between px-4 gap-10 items-center md:items-stretch flex flex-col-reverse md:flex-row">
        <div className="flex flex-col justify-center relative">
          <div className="hidden md:block text-start">
            <p className="text-[56px] font-playfair font-[500]">swa promise</p>
            <p className="text-[20px] mt-3 font-[400] text-[#334155] leading-[32px]">
              With timeless charm and radiant grace, Preity <br />
              Zinta embodies the spirit of our jewellery — where <br /> every
              diamond tells a story,
            </p>
          </div>
          <img
            src="/try/diaRing.svg"
            className="w-[200px] md:w-[337.94px] h-[120px] md:h-[203.12px] static md:absolute bottom-0 z-100"
            alt="ring"
          />
        </div>

        <div className="grid lg:grid-cols-4 grid-cols-3 gap-2 md:gap-8 text-[#334155] py-4 md:py-20 max-w-[750px]">
          {data.map((item, index) => (
            <div className="text-center flex flex-col items-center" key={index}>
              <div className="w-[90px] h-[90px] md:w-[160px] md:h-[160px] bg-white rounded-full mb-2 flex items-center justify-center">
                <img
                  src={item.image}
                  alt={item.title}
                  className="w-[40px] md:w-[64px] h-[40px] md:h-[64px]"
                />
              </div>
              <p className="text-[0.75rem] md:text-[14px] font-[400] leading-[20px]">
                {item.title}
              </p>
            </div>
          ))}
        </div>

        <div className="block md:hidden text-center mt-4">
          <p className="text-4xl font-playfair">Swa promise</p>
          <p className="text-[0.75rem] mt-3 font-[400] text-[#334155]">
            With timeless charm and radiant grace, Preity <br />
            Zinta embodies the spirit of our jewellery — where <br /> every
            diamond tells a story,
          </p>
        </div>
      </div>
    </div>
  );
};

export default SwaPromise;
